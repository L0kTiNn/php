...
- Если она была отправлена: отфильтруйте полученные данные
  с помощью функций trim(), htmlspecialchars(), strip_tags() и mysqli_real_escape_string(),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $name = trim(strip_tags(mysqli_real_escape_string($connection, $_POST['name'] ?? '')));
  $email = trim(htmlspecialchars(mysqli_real_escape_string($connection, $_POST['email'] ?? '')));
  $msg = trim(mysqli_real_escape_string($connection, $_POST['msg'] ?? ''));

  
  $tidy = tidy_parse_string($msg, ['output-xhtml' => true, 'wrap' => 0], 'UTF8');
  $tidy->cleanRepair();
  $msg = (string) $tidy;
